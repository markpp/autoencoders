...
def create_encoder(config):
    return nn.Sequential(
        ...
        nn.LeakyReLU(True),
        # input (nfe*8) x 8 x 8
        nn.Conv2d(config['model_params']['nfe'] * 8, config['model_params']['nfe'] * 16, 4, 2, 1, bias=False),
        nn.BatchNorm2d(config['model_params']['nfe'] * 16),
        nn.LeakyReLU(True),
        # input (nfe*16) x 4 x 4
        nn.Conv2d(config['model_params']['nfe'] * 16, config['model_params']['latent_dim'], 4, 1, 0, bias=False),
...
def create_decoder(config):
    return nn.Sequential(
        ...
        nn.BatchNorm2d(config['model_params']['nfd'] * 16),
        nn.ReLU(True),
        # input (nfd*16) x 4 x 4
        nn.ConvTranspose2d(config['model_params']['nfd'] * 16, config['model_params']['nfd'] * 8, 4, 2, 1, bias=False),
