...
class MyDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.data_train = SewerDataset(os.path.join(hparams.data_path,'train'),
                                           transforms=self.train_t,
                                           n_channels=hparams.in_channels)
            self.data_val = SewerDataset(os.path.join(hparams.data_path,'val'),
    ...
    def val_dataloader(self):
        return DataLoader(self.data_val, batch_size=hparams.batch_size, shuffle=False, num_workers=hparams.n_workers, pin_memory=True)
