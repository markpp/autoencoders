...
def get_variate_masks(stats):
    thresh = np.quantile(stats, 1 - hparams.variate_masks_quantile)
    return stats > thresh
...
def effective_pixels():
    if hparams.dataset_source == 'binarized_mnist':
        return 28 * 28 * hparams.channels
    else:
        return hparams.target_res * hparams.target_res * hparams.channels
...
def create_checkpoint_manager_and_load_if_exists(model_directory='.', rank=0):
    checkpoint_path = os.path.join(model_directory, f'checkpoints-{hparams.name}')
...
def compute_latent_dimension():
    assert np.prod(hparams.down_strides) == np.prod(hparams.up_strides)

    return hparams.target_res // np.prod(hparams.down_strides)
