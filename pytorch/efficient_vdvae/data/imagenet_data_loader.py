...
def read_imagenet_images(path):
    ...
    data = data.reshape(
        [data.shape[0], hparams.channels, hparams.target_res, hparams.target_res])  # [samples, C, H, W]

    data = data.transpose([0, 2, 3, 1])  # [samples, H, W, C]
    assert data.shape[1] == data.shape[2] == hparams.target_res
    assert data.shape[3] == hparams.channels
...
def train_val_data_imagenet(world_size, rank):
    train_images = read_imagenet_images(hparams.train_data_path)
    ...
    train_loader = torch.utils.DataLoader(sampler=train_sampler,
    ...
    val_images = read_imagenet_images(hparams.val_data_path)
    ...
    val_loader = torch.utils.DataLoader(sampler=val_sampler,
...
def synth_imagenet_data():
    synth_images = read_imagenet_images(hparams.synthesis_data_path)
    synth_images = imagenet_dataset(synth_images, mode='test')
    synth_loader = torch.utils.DataLoader(
...
def encode_imagenet_data():
    images = read_imagenet_images(hparams.train_data_path)
    images = imagenet_dataset(images, mode='encode')
    encode_loader = torch.utils.DataLoader(
        dataset=images,
        batch_size=hparams.synthesis.batch_size,
        shuffle=True,
        pin_memory=True,
...
def stats_imagenet_data():
    ...
    stats_loader = torch.utils.DataLoader(
        dataset=images,
        batch_size=hparams.synthesis.batch_size,
        shuffle=True,
        pin_memory=True,
