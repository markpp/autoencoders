...
class UniversalAutoEncoder(nn.Module):
    ...
    def forward(self, x, variate_masks=None):
        ...
        if variate_masks is None:
            variate_masks = [None] * (sum(hparams.down_n_blocks_per_res) + len(hparams.down_strides))
        else:
            assert len(variate_masks) == sum(hparams.down_n_blocks_per_res) + len(hparams.down_strides)
