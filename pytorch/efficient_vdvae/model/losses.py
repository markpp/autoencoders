...
def _compute_inv_stdv(logits):
    ...
    if hparams.output_distribution_base == 'std':
        scales = torch.maximum(softplus(logits),
                               torch.as_tensor(np.exp(hparams.min_mol_logscale)))
        ...
        inv_stdv = torch.exp(-hparams.output_gradient_smoothing_beta * log_scales)
    else:
...
class DiscMixLogistic(nn.Module):
    def __init__(self):
        ...
        self.num_classes = 2. ** hparams.num_bits - 1.
    ...
    def forward(self, targets, logits, global_batch_size):
        ...
        inv_stdv, log_scales = _compute_inv_stdv(
            l[:, :, hparams.num_output_mixtures: 2 * hparams.num_output_mixtures, :, :])
        ...
        broadcast_targets = torch.broadcast_to(targets, size=[B, C, hparams.num_output_mixtures, H, W])
        ...
        avg_per_example_loss = per_example_loss / (
                np.prod([negative_log_probs.size()[i] for i in mean_axis]) * hparams.channels)  # B

        assert len(per_example_loss.size()) == len(avg_per_example_loss.size()) == 1

        scalar = global_batch_size * hparams.target_res * hparams.target_res * hparams.channels
...
def calculate_logstd_loss(p, q):
    ...
    p_std = torch.exp(hparams.gradient_smoothing_beta * p_logstd)
    inv_q_std = torch.exp(-hparams.gradient_smoothing_beta * q_logstd)
...
class KLDivergence(nn.Module):
    def forward(self, p, q, global_batch_size):
        if hparams.distribution_base == 'std':
            loss = calculate_std_loss(p, q)
        elif hparams.distribution_base == 'logstd':
            loss = calculate_logstd_loss(p, q)
        else:
            raise ValueError(f'distribution base {hparams.distribution_base} not known!!')
