...
def reconstruction_mode(artifacts_folder, latents_folder, test_dataset, model, ssim_metric):
    ...
    if hparams.mask_reconstruction:
    ...
    for step, inputs in enumerate(test_dataset):
        ...
        ssim_per_batch = ssim_metric(targets, outputs, global_batch_size=hparams.synthesis_batch_size)
        ssims += ssim_per_batch
        nelbos += nelbo

        # Save images to disk
        for batch_i, (target, output) in enumerate(zip(targets, outputs)):
            if hparams.save_target_in_reconstruction:
                write_image_to_disk(
...
def generation_mode(artifacts_folder, latents_folder, model):
    ...
    for temp_i, temperature_setting in enumerate(hparams.temperature_settings):
        ...
        if isinstance(temperature_setting, list):
            ...
            assert len(temperature_setting) == len(hparams.down_strides)
            ...
            temperatures = [temperature_setting] * len(hparams.down_strides)
            ...
            temp_fn = getattr(temperature_functions, temperature_setting[0])(temperature_setting[1],
                    ...
                    hparams.down_strides))
            temperatures = [temp_fn(layer_i) for layer_i in range(len(hparams.down_strides))]

        else:
            raise ValueError(f'Temperature Setting {temperature_setting} not interpretable!!')

        sample_i = 0
        for step in range(hparams.n_generation_batches):
...
def compute_per_dimension_divergence_stats(dataset, model, latents_folder):
    ...
    with torch.no_grad():
        for step, inputs in enumerate(tqdm(dataset)):
            ...
            kl_div = kl_stats(posterior_dist_list, prior_kl_dist_list, hparams.synthesis_batch_size)
...
def encoding_mode(latents_folder, dataset, model):
    ...
    with open(os.path.join(latents_folder, f'encodings_seed_{hparams.seed}.pkl'), 'wb') as handle:
...
def synth_data():
    ...
    elif hparams.dataset_source == 'binarized_mnist':
...
def main():
    ...
    if hparams.synthesis_mode == 'reconstruction':
        data_loader = synth_data()
    elif hparams.synthesis_mode == 'encoding':
        data_loader = encode_data()
    elif hparams.synthesis_mode == 'div_stats':
        data_loader = stats_data()
    elif hparams.synthesis_mode == 'generation':
        data_loader = None
    else:
        raise ValueError(f'Unknown Mode {hparams.synthesis_mode}')

    # Synthesis using pretrained model
    logdir = get_logdir()
    synthesize(model, data_loader, logdir, mode=hparams.synthesis_mode)
